...
describe("[Challenge] Backdoor", function () {
  ...
  before(async function () {
    ...
    for (let i = 0; i < users.length; i++) {
      ...
      // checking balance of users
      expect(await token.balanceOf(users[i])).to.eq(0);

      // checking the wallets of the users
      let wallet = await walletRegistry.wallets(users[i]);
      expect(wallet).to.eq(ethers.constants.AddressZero);
    }
